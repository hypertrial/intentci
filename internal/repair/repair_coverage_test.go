...
func TestRepairVerifyErrorAndRepeatedFailure(t *testing.T) {
	...
	out, err := repair.Run(context.Background(), repair.Options{
		Root: root, Config: cfg, Store: store, DryRun: true, MaxAttempts: 3,
...
func TestRepairAgentProtectedTestAndRepeatedDiff(t *testing.T) {
	...
	b := failBundle("rp")
	out, err := repair.Run(context.Background(), repair.Options{
		Root: root, Config: cfg, Store: store, AgentCommand: agent + " # {packet}", MaxAttempts: 2,
		Verify: func(ctx context.Context) (*evidence.Bundle, error) { return b, nil },
	})
	if err != nil {
		t.Fatal(err)
	...
	b2 := failBundle("rt")
	out, err = repair.Run(context.Background(), repair.Options{
		Root: root2, Config: cfg, Store: store2, AgentCommand: agent2, MaxAttempts: 2,
	...
	n := 0
	out, err = repair.Run(context.Background(), repair.Options{
		Root: root3, Config: cfg3, Store: store3, AgentCommand: agent3, MaxAttempts: 4,
		Verify: func(ctx context.Context) (*evidence.Bundle, error) {
			n++
			bb := failBundle("rd")
			bb.RunID = "rd"
	...
	root4 := t.TempDir()
	gitInit(t, root4)
	gitCommitTree(t, root4, map[string]string{"README.md": "x\n"})
	store4, _ := evidence.NewStore(root4, t.TempDir())
	_ = os.WriteFile(filepath.Join(store4.Root, "rw"), []byte("x"), 0o644)
	bb := failBundle("rw")
	out, err = repair.Run(context.Background(), repair.Options{
		Root: root4, Config: config.Default(), Store: store4, DryRun: true, MaxAttempts: 1,
...
func TestRepairMaxAttemptsDefault(t *testing.T) {
	...
	attempts := 0
	out, err := repair.Run(context.Background(), repair.Options{
		Root: root, Config: cfg, Store: store, DryRun: true, AgentCommand: "",
		Verify: func(ctx context.Context) (*evidence.Bundle, error) {
			attempts++
