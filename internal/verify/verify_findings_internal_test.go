...
func TestRun_DefaultProfileAndFindingsNil(t *testing.T) {
	dir := setupRepo(t)
	body := `version: 1
...
`
	...
	if o.Result.ChangeFindings == nil {
		t.Fatal("findings slice")
	}
...
`
	...
	o, err = Run(context.Background(), Options{Root: dir, Base: "HEAD", ChangeID: "C-1", Trust: true})
	if err != nil {
		t.Fatal(err)
	}
	if len(o.Result.ChangeFindings) == 0 {
		t.Fatal("expected change findings")
	}
	_ = filepath.Base
}
