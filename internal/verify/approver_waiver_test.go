...
func TestRun_WaiverApproverOnly(t *testing.T) {
	dir := t.TempDir()
...
waivers:
  - id: W-002
    ...
    expires: ` + exp + `
`
	if err := os.WriteFile(filepath.Join(dir, ".intentci", "changes", "WTEST-2.yaml"), []byte(change), 0o644); err != nil {
		t.Fatal(err)
	}
	run("git", "add", ".")
	...
	out, err := verify.Run(context.Background(), verify.Options{
		...
		ChangeID: "WTEST-2", Stdout: &stdout, Stderr: &stderr,
	})
	if err != nil {
		t.Fatal(err)
	}
	if out.Result.Status != protocol.StatusPass {
		t.Fatalf("%s %+v", out.Result.Status, out.Result.Requirements)
	}
}
