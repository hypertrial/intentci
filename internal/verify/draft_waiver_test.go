...
func TestRun_DraftWaiverIgnored(t *testing.T) {
	dir := t.TempDir()
...
waivers:
  - id: W-001
    ...
    expires: ` + exp + `
`
	if err := os.WriteFile(filepath.Join(dir, ".intentci", "changes", "DRAFT-1.yaml"), []byte(change), 0o644); err != nil {
		t.Fatal(err)
	}
	run("git", "add", ".")
	...
	out, err := verify.Run(context.Background(), verify.Options{
		...
		ChangeID: "DRAFT-1", Attest: true, Stdout: &stdout, Stderr: &stderr,
	})
	if err != nil {
		t.Fatal(err)
	}
	if out.Result.Status != protocol.StatusFail || out.ExitCode != 10 {
		t.Fatalf("draft waiver must not skip failures: status=%s code=%d", out.Result.Status, out.ExitCode)
	}
	if out.AttestationPath != "" {
		t.Fatal("must not attest non-pass")
	}
	for _, r := range out.Result.Requirements {
		if r.ID == "BUILD-001" && r.Status == protocol.ReqWaived {
			t.Fatal("draft waiver applied")
		}
	}
}
